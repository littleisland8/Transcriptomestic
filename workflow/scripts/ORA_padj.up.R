...
outputdir <- "/projects2/2024_Chiarugi/Transcriptomestic/results/unstranded/STAR_HTSeq_nosample1/ORA_padj_UP"
...
if (length(which(C7$p.adjust < 0.05)) > 10){
  ...
  p1 <- treeplot(edox2, label_format = 4, showCategory = 10,nCluster=6, fontsize=1.5)
  ggsave(file.path(C7path, "clust.wardD2.pdf"), height = 10, width = 12)
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, showCategory=10,nCluster=6, fontsize=1.5)
