...
outputdir <- "/projects2/2024_Chiarugi/Transcriptomestic/results/unstranded/STAR_HTSeq_nosample1/ORA_padj_FC"
...
df <- df[which(abs(as.numeric(df$log2FoldChange)) > 1 & df$padj < 0.05 | df$padj==0),]
...
for (ont in ontologies){
  ...
  if (length(which(ego@result$p.adjust < 0.05)) > 10){
    ...
    p <- barplot(ego, showCategory = 20, colorby = 'pvalue', font.size = 8) + ggtitle("GO Barplot")
    ggsave(file.path(outdir_,"barplot.pdf"))
    
    #dotplot show most significant enriched terms 
    message(paste0("Drawing dotplot"))
    p <-dotplot(ego, showCategory = 20) + ggtitle("GO Dotplot")
    ...
    p1 <- cnetplot(ego, showCategory = 20) + ggtitle("GO Gene-concept Network plot") #category size scaled by gene number 
    ...
    p1 <- heatplot(ego, showCategory=10)
    p2 <- heatplot(ego, foldChange=genelist, showCategory=10) #+ theme(axis.text.x = element_text(size = 4, angle = 90))
    cowplot::plot_grid(p1, p2, ncol=1, labels=LETTERS[1:2])
    ggsave(file.path(outdir_, "heatmap.pdf"), height = 10, width = 15)
...
if (length(which(x@result$p.adjust < 0.05)) > 10){
  ...
  #barplot
  message(paste0("Drawing barplot"))
  p <- barplot(x, showCategory = 20, colorby = 'pvalue') + ggtitle("REACTOME Barplot")
  ggsave(file.path(reactomepath,"barplot.pdf"))
  
  #dotplot show most significant enriched terms 
  message(paste0("Drawing dotplot"))
  p <- dotplot(x, showCategory = 20) + ggtitle("REACTOME Dotplot")
...
if (length(which(kks_table$p.adjust < 0.05)) > 10){
  ...
  #barplot
  message(paste0("Drawing barplot"))
  p <- barplot(kks, showCategory = 20, colorby = 'pvalue') + ggtitle("KEGG Barplot")
  ggsave(file.path(KEGGoutdir,"barplot.pdf"))
  
  #dotplot show most significant enriched terms 
  message(paste0("Drawing dotplot"))
  p <- dotplot(kks, showCategory = 20) + ggtitle("KEGG Dotplot")
...
## Wiki
wiki <- enrichWP(names(genelist), organism = "Homo sapiens", pvalueCutoff = 0.05, qvalueCutoff = 0.05, minGSSize = 5,maxGSSize = 500)
...
if (length(which(x$p.adjust < 0.05)) > 10){
  ...
  #dotplot show most significant enriched terms 
  message(paste0("Drawing dotplot"))
  p <- dotplot(x, showCategory = 10) + ggtitle("MeSH gendoo Dotplot")
  ggsave(file.path(dopath, "dotplot.pdf"))
  ...
  #Heatmap-like functional classification
  p1 <- heatplot(x, showCategory=5)
  p2 <- heatplot(x, foldChange=genelist, showCategory=5) #+ theme(axis.text.x = element_text(size = 4, angle = 90))
  cowplot::plot_grid(p1, p2, ncol=1, labels=LETTERS[1:2])
  ggsave(file.path(dopath, "heatmap.pdf"))
  ...
  ggsave(file.path(dopath, "clust.wardD2.pdf"))
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, showCategory = 10,nCluster=1)
  ggsave(file.path(dopath, "clust.average.pdf"))
  
  #emapplot show different genesets connected by overlapping genes. Overlapping gene sets are tend to cluster together 
  message(paste0("Drawing emapplot for"))
  emapplot(edox2, showCategory = 10, node_scale = 2)  + ggtitle("MeSH gendoo Enrichment Map plot")
  ggsave(file.path(dopath, "emapplot.pdf"))
...
if (length(which(H$p.adjust < 0.05)) > 10){
  ...
  #barplot
  message(paste0("Drawing barplot"))
  p <- barplot(H, showCategory = 20, colorby = 'pvalue') + ggtitle("HALLMARK Barplot")
  ggsave(file.path(Hpath,"barplot.pdf"))
  
  #dotplot show most significant enriched terms 
  message(paste0("Drawing dotplot"))
  p <- dotplot(H, showCategory = 20) + ggtitle("HALLMARK Dotplot")
  ...
  #Heatmap-like functional classification
  p1 <- heatplot(H, showCategory=10)
  p2 <- heatplot(H, foldChange=genelist, showCategory=10) #+ theme(axis.text.x = element_text(size = 4, angle = 90))
  cowplot::plot_grid(p1, p2, ncol=1, labels=LETTERS[1:2])
  ggsave(file.path(Hpath, "heatmap.pdf"))
...
if (length(which(x@result$p.adjust < 0.05)) > 10){
  ...
  #barplot
  message(paste0("Drawing barplot"))
  p <- barplot(x, showCategory = 20, colorby = 'pvalue') + ggtitle("DGN Barplot")
  ggsave(file.path(DGNpath,"barplot.pdf"))
  
  #dotplot show most significant enriched terms 
  message(paste0("Drawing dotplot"))
  p <- dotplot(x, showCategory = 20) + ggtitle("DGN Dotplot")
  ...
  p2 <- heatplot(x, foldChange=genelist, showCategory=10) #+ theme(axis.text.x = element_text(size = 4, angle = 90))
  cowplot::plot_grid(p1, p2, ncol=1, labels=LETTERS[1:2])
  ggsave(file.path(DGNpath, "heatmap.pdf"))
  
  #Tree plot
  edox2 <- pairwise_termsim(x)
  p1 <- treeplot(edox2, label_format = 4, showCategory = 20, nCluster=5)
  ggsave(file.path(DGNpath, "clust.wardD2.pdf"))
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, nCluster=7)
...
if (length(which(C2$p.adjust < 0.05)) > 10){
  ...
  #barplot
  message(paste0("Drawing barplot"))
  p <- barplot(C2, showCategory = 20, colorby = 'pvalue') + ggtitle("C2 Barplot")
  ggsave(file.path(C2path,"barplot.pdf"))
  
  #dotplot show most significant enriched terms 
  message(paste0("Drawing dotplot"))
  p <- dotplot(C2, showCategory = 20) + ggtitle("C2 Dotplot")
  ...
  #Heatmap-like functional classification
  p1 <- heatplot(C2, showCategory=10)
  p2 <- heatplot(C2, foldChange=genelist, showCategory=10) #+ theme(axis.text.x = element_text(size = 4, angle = 90))
  cowplot::plot_grid(p1, p2, ncol=1, labels=LETTERS[1:2])
  ggsave(file.path(C2path, "heatmap.pdf"), height = 10, width = 15)
  ...
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, fontsize=1.5,nCluster=8)
  ggsave(file.path(C2path, "clust.average.pdf"), height=15, width=12)
...
if (length(which(C6$p.adjust < 0.05)) > 10){
  ...
  #barplot
  message(paste0("Drawing barplot"))
  p <- barplot(C6, showCategory = 20, colorby = 'pvalue') + ggtitle("C6 Barplot")
  ggsave(file.path(C6path,"barplot.pdf"))
  
  #dotplot show most significant enriched terms 
  message(paste0("Drawing dotplot"))
  p <- dotplot(C6, showCategory = 20) + ggtitle("C6 Dotplot")
  ...
  #Heatmap-like functional classification
  p1 <- heatplot(C6, showCategory=10)
  p2 <- heatplot(C6, foldChange=genelist, showCategory=10) #+ theme(axis.text.x = element_text(size = 4, angle = 90))
  ggsave(file.path(C6path, "heatmap.pdf"))
  
  #Tree plot
  edox2 <- pairwise_termsim(C6)
  p1 <- treeplot(edox2, label_format = 4, showCategory = 20,nCluster=5)
  ggsave(file.path(C6path, "clust.wardD2.pdf"))
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4,nCluster=5)
