...
outputdir <- "/projects2/2024_Chiarugi/Transcriptomestic/results/unstranded/STAR_HTSeq_nosample1/ORA_padj_DOWN"
...
df <- df[which(df$log2FoldChange < 0 & df$padj < 0.05 | df$padj==0),]
...
for (ont in ontologies){
  ...
  if (length(which(ego@result$p.adjust < 0.05)) > 10){
    ...
    p <-emapplot(edox2, showCategory = 10, node_scale = 2)  + ggtitle("GO Enrichment Map plot")
...
if (length(which(kks_table$p.adjust < 0.05)) > 10){
  ...
  p1 <- treeplot(edox2, label_format = 4, showCategory = 10,nCluster=5, fontsize=3)
  ggsave(file.path(KEGGoutdir, "clust.wardD2.pdf"))
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 3,nCluster=5, fontsize=3)
...
if (length(which(wiki_table$p.adjust < 0.05)) > 10){
  ...
  p1 <- treeplot(edox2, label_format = 4, showCategory = 10,nCluster=4, fontsize=4)
  ggsave(file.path(wikipath, "clust.wardD2.pdf"))
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, showCategory = 10,nCluster=4, fontsize=4)
...
if (length(which(x$p.adjust < 0.05)) > 10){
  ...
  ggsave(file.path(meshpath,"barplot.pdf"))
  
  #dotplot show most significant enriched terms 
  message(paste0("Drawing dotplot"))
  p1 <- dotplot(x, showCategory = 10) + ggtitle("MeSH gendoo Dotplot")
  ggsave(file.path(meshpath, "dotplot.pdf"))
  
  #cnetplot show genes involved in significant terms. In order to consider the potentially biological complexities in which a gene may belong to multiple annotation categories and provide information of numeric changes if available, cnetplot function extracts the complex association 
  message(paste0("Drawing cnetplot p1 for"))
  p1 <- cnetplot(x, showCategory = 10) + ggtitle("MeSH gendoo Gene-concept Network plot") #category size scaled by gene number 
  ggsave(file.path(meshpath, "cnetplot.1.pdf"))
  ...
  ggsave(file.path(meshpath, "cnetplot.2.pdf"))
  ...
  ggsave(file.path(meshpath, "cnetplot.3.pdf"))
  ...
  cowplot::plot_grid(p1, p2, ncol=1, labels=LETTERS[1:2])
  ggsave(file.path(meshpath, "heatmap.pdf"), height = 12, width=20)
  
  #Tree plot
  edox2 <- pairwise_termsim(x)
  p1 <- treeplot(edox2, label_format = 4, showCategory = 10,nCluster=4)
  ggsave(file.path(meshpath, "clust.wardD2.pdf"))
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, showCategory = 10,nCluster=4)
  ggsave(file.path(meshpath, "clust.average.pdf"))
  
  #emapplot show different genesets connected by overlapping genes. Overlapping gene sets are tend to cluster together 
  message(paste0("Drawing emapplot for"))
  p <- emapplot(edox2, showCategory = 10, node_scale = 2)  + ggtitle("MeSH gendoo Enrichment Map plot")
  ggsave(file.path(meshpath, "emapplot.pdf"))
  
  #UpSet Plot
  pdf(file.path(meshpath,"upset.pdf"))
  upsetplot(x) + theme(plot.margin = unit(c(1,1,1,4), "cm"), axis.text.y = element_text(size = 15/.pt))
  dev.off()   
} 
...
if (length(which(H$p.adjust < 0.05)) > 10){
  ...
  p1 <- treeplot(edox2, label_format = 4, showCategory = 10, fontsize=3,nCluster = 4)
  ggsave(file.path(Hpath, "clust.wardD2.pdf"))
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, showCategory = 10, fontsize=3,nCluster = 4)
  ...
  #UpSet Plot
  pdf(file.path(Hpath,"upset.pdf"))
  upsetplot(H) + theme(plot.margin = unit(c(1,1,1,4), "cm"), axis.text.y = element_text(size = 15/.pt))
  dev.off()   
...
if (length(which(x@result$p.adjust < 0.05)) > 10){
  ...
  #UpSet Plot
  pdf(file.path(NCGpath,"upset.pdf"))
  upsetplot(x) + theme(plot.margin = unit(c(1,1,1,4), "cm"), axis.text.y = element_text(size = 15/.pt))
  dev.off()   
}
...
if (length(which(x@result$p.adjust < 0.05)) > 10){
  ...
  p2 <- treeplot(edox2, hclust_method = "average", showCategory = 10, nCluster=3)
  ...
  #UpSet Plot
  pdf(file.path(DGNpath,"upset.pdf"))
  upsetplot(x) + theme(plot.margin = unit(c(1,1,1,4), "cm"), axis.text.y = element_text(size = 15/.pt))
  dev.off()   
}
...
if (length(which(C2$p.adjust < 0.05)) > 10){
  ...
  p1 <- treeplot(edox2, label_format = 4, fontsize=2.0,showCategory = 10,nCluster=4)
  ggsave(file.path(C2path, "clust.wardD2.pdf"), height=12, width=15)
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, fontsize=2.0,showCategory = 10,nCluster=4)
  ggsave(file.path(C2path, "clust.average.pdf"), height=12, width=15)
  ...
  #UpSet Plot
  pdf(file.path(C2path,"upset.pdf"))
  upsetplot(C2) + theme(plot.margin = unit(c(1,1,1,4), "cm"), axis.text.y = element_text(size = 15/.pt))
  dev.off()   
}
#C6
...
if (length(which(C6$p.adjust < 0.05)) > 10){
  ...
  p1 <- treeplot(edox2, label_format = 4, showCategory = 10,nCluster=2)
  ggsave(file.path(C6path, "clust.wardD2.pdf"), height = 10, width = 12)
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, showCategory=10,nCluster=2)
  ggsave(file.path(C6path, "clust.average.pdf"), height = 10, width = 12)
  ...
  #UpSet Plot
  pdf(file.path(C6path,"upset.pdf"))
  upsetplot(C6) + theme(plot.margin = unit(c(1,1,1,4), "cm"), axis.text.y = element_text(size = 15/.pt))
  dev.off()   
}
...
## C7
C7 <- enricher(names(genelist), TERM2GENE = m_t2g_C7, minGSSize = 2,maxGSSize = 500,pvalueCutoff  = 0.05, qvalueCutoff  = 0.05)
C7 <- setReadable(C7, 'org.Hs.eg.db', 'ENTREZID')

C7path <- paste(file.path(Msigdbrpath, "C7"))
cmd <- paste("mkdir -p", C7path)
system(cmd)

C7_table <- as.data.frame(C7@result) 
write.table(C7_table,file.path(C7path,"C7_table.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
write.csv(C7_table,file.path(C7path,"C7_table.csv"), row.names = FALSE, quote = FALSE)
#write.xlsx(C6_table,file.path(C6path,"C6_table.xlsx"), row.names = FALSE)


if (length(which(C7$p.adjust < 0.05)) > 10){
  
  ##Plot
  
  #barplot
  message(paste0("Drawing barplot"))
  p <- barplot(C7, showCategory = 10, colorby = 'pvalue') + ggtitle("C6 Barplot")
  ggsave(file.path(C7path,"barplot.pdf"))
  
  #dotplot show most significant enriched terms 
  message(paste0("Drawing dotplot"))
  p <- dotplot(C7, showCategory = 10) + ggtitle("C6 Dotplot")
  ggsave(file.path(C7path, "dotplot.pdf"))
  
  #cnetplot show genes involved in significant terms. In order to consider the potentially biological complexities in which a gene may belong to multiple annotation categories and provide information of numeric changes if available, cnetplot function extracts the complex association 
  message(paste0("Drawing cnetplot p1 for"))
  p1 <- cnetplot(C7, showCategory = 10) + ggtitle("C6 Gene-concept Network plot") #category size scaled by gene number 
  ggsave(file.path(C7path, "cnetplot.1.pdf"))
  
  ## categorySize can be scaled by 'pvalue' or 'geneNum'
  message(paste0("Drawing cnetplot p2"))
  p2 <- cnetplot(C7, categorySize="pvalue", showCategory = 10,foldChange=genelist) + ggtitle("C6 Gene-concept Network scaled by pvalue")
  ggsave(file.path(C7path, "cnetplot.2.pdf"))
  
  message(paste0("Drawing cnetplot p3"))
  p3 <- cnetplot(C7, circular = TRUE, colorEdge = TRUE, showCategory = 10,foldChange=genelist) + ggtitle("C6 Gene-concept Network circular")
  ggsave(file.path(C7path, "cnetplot.3.pdf"))
  
  #cowplot::plot_grid(p1,p2, p3, ncol=3, labels=LETTERS[1:2])
  #ggsave(file.path(reactomepath, "cnetplot.total.pdf"))
  
  #Heatmap-like functional classification
  p1 <- heatplot(C7, showCategory=5) + theme(axis.text.x = element_text(size = 2.5, angle = 90))
  p2 <- heatplot(C7, foldChange=genelist, showCategory=5) + theme(axis.text.x = element_text(size = 2.5, angle = 90))
  cowplot::plot_grid(p1, p2, ncol=1, labels=LETTERS[1:2])
  ggsave(file.path(C7path, "heatmap.pdf"), height = 12, width=20)
  
  #Tree plot
  edox2 <- pairwise_termsim(C7)
  p1 <- treeplot(edox2, label_format = 4, showCategory = 10,nCluster=4, fontsize=1.5)
  ggsave(file.path(C7path, "clust.wardD2.pdf"), height = 10, width = 12)
  p2 <- treeplot(edox2, hclust_method = "average",label_format = 4, showCategory=10,nCluster=4, fontsize=1.5)
  ggsave(file.path(C7path, "clust.average.pdf"), height = 10, width = 12)
  
  #emapplot show different genesets connected by overlapping genes. Overlapping gene sets are tend to cluster together 
  message(paste0("Drawing emapplot for"))
  p <- emapplot(edox2, showCategory = 10, node_scale = 2)  + ggtitle("C6 Enrichment Map plot")
  ggsave(file.path(C7path, "emapplot.pdf"))
  
  #UpSet Plot
  pdf(file.path(C7path,"upset.pdf"))
  upsetplot(C7) + theme(plot.margin = unit(c(1,1,1,4), "cm"), axis.text.y = element_text(size = 15/.pt))
  dev.off()   
}
